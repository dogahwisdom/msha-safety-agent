...
classification, 20 trend, 20 case-grounded), the tool-augmented agent
achieves \textbf{93.3\% overall accuracy} in offline tool routing mode,
versus \textbf{30.0\%} for each baseline, with \textbf{100\% tool
selection correctness} and mean latency \textbf{0.26\,s} per question.
...
\subsection{Benchmark accuracy (60 questions)}

Three systems were evaluated on the same 60 questions: tool-augmented agent
(offline routing), classifier baseline, and retrieval-only baseline (semantic
search without LLM synthesis).

\begin{table}[htbp]
  \centering
  \caption{Benchmark accuracy by question category ($n=20$ per category).}
  \label{tab:benchmark}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    System & Classification & Trend & Case-grounded & Overall \\
    \midrule
    Tool-augmented agent & 90.0\% & \textbf{100.0\%} & 90.0\% & \textbf{93.3\%} \\
...
The agent's advantage over baselines reflects \emph{coverage}: each baseline
handles only one question type, while the agent routes to the appropriate tool
for all three. Tool selection correctness for the offline agent was
\textbf{100\%} (60/60), because routing uses benchmark category metadata; live
LLM evaluation must infer tool choice from natural language alone.

Mean latency: agent 0.26\,s, classifier baseline 0.18\,s, retrieval baseline
0.03\,s. No LLM tokens were consumed in the offline run.
...
the University of Mines and Technology is planned.

\subsection{Live LLM evaluation (Ollama, local)}
\label{sec:live-llm}

A supplementary run used Ollama with \texttt{qwen2.5:7b} on the same 60-question
benchmark:

\begin{table}[htbp]
  \centering
  \caption{Offline tool routing vs.\ local live LLM (Ollama).}
  \label{tab:ollama}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    ...
    Offline tool agent & \textbf{93.3\%} & \textbf{100\%} & 0.26\,s \\
    \bottomrule
  \end{tabular}
\end{table}

The smaller local model struggled on trend questions (0\% accuracy) and
misread some tool outputs. Larger models with reliable function calling are
recommended for live deployment; see the repository reproduction guide.
...
60-question benchmark, the tool-augmented agent achieves 93.3\% accuracy in
offline routing mode versus 30.0\% for each single-tool baseline, demonstrating
that multi-tool coverage is necessary for mixed natural language safety
questions. Third, it prepares the first human evaluation protocol for
explanation quality in mine safety AI using the Explanation Satisfaction Scale
of Hoffman et al.~\cite{Hoffman2023}; participant data collection remains
future work.

Limitations stated plainly:
\begin{itemize}
  \item \textbf{Offline routing:} Primary benchmark numbers use category-based
    tool routing, not live LLM natural language understanding.
  ...
  \item \textbf{Retrieval:} Semantic search sometimes returns plausible but
    incorrect reference documents (two benchmark failures).
  \item \textbf{Human evaluation:} Protocol prepared; participant data not yet
    collected.
  \item \textbf{Cost of live LLM:} Not required for reproduction; Groq free tier
    and Ollama are supported alternatives to paid OpenAI.
\end{itemize}
