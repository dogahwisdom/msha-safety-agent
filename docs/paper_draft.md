...
All three systems attempt all 60 questions. The live agent exceeds both single-tool baselines on overall accuracy but remains weak on trend questions (5%), where tool outputs require correct numeric parsing from natural language. It **underperforms the classifier baseline on classification questions** (85.0% vs 90.0%) and **underperforms the RAG baseline on case grounded questions** (25.0% vs 85.0%); the overall 38.3% figure reflects breadth across categories, not dominance on any one type.

**McNemar's test (paired per-question correctness, n=60).** Comparing the Groq agent against the classifier baseline: 23 vs 18 questions correct overall; discordant pairs were 6 agent-only correct and 1 classifier-only correct (McNemar χ²=2.29, p=0.131). Comparing the agent against the RAG baseline: 23 vs 17 correct; discordant pairs were 18 agent-only correct and 12 RAG-only correct (McNemar χ²=0.83, p=0.361). Neither comparison reaches conventional significance at α=0.05 given n=60. The observed overall differences (38.3% vs 30.0% vs 28.3%) are **suggestive but not statistically conclusive** at this sample size. Reproduce with `python eval/significance_test.py`.
...
This paper makes three contributions, now supported by implemented code and measured results. First, it introduces a tool augmented LLM agent architecture for reasoning over the MSHA accident and injury dataset, combining a structured classifier, a statistical trend tool, and a retrieval tool over incident narratives, following the transparent, framework free orchestration design demonstrated by Lu (2026) in a different industrial domain. Second, on a fixed 60 question benchmark where every system attempts every question, the live Groq agent achieves 38.3% overall accuracy versus 30.0% for the classifier baseline and 28.3% for the RAG baseline, with tool selection correctness at 60%; McNemar's tests at n=60 do not show statistically significant paired differences at α=0.05 (p=0.131 vs classifier, p=0.361 vs RAG). The agent underperforms the classifier on classification questions (85.0% vs 90.0%) and underperforms the RAG baseline on case grounded questions (25.0% vs 85.0%); its overall lead comes from attempting all three categories rather than excelling on any single one. Third, it prepares the first human evaluation protocol for explanation quality in mine safety AI using the Explanation Satisfaction Scale of Hoffman et al. (2023); participant data collection has not started pending UMaT ethics confirmation.
...
- **Small sample and single Groq run:** The primary Groq benchmark reflects one completed run. LLM outputs are not fully deterministic; repeated runs could shift the reported numbers, particularly with only 60 questions. McNemar's tests did not support statistically significant overall differences at α=0.05.
...
- **Live LLM weakness:** Primary Groq agent accuracy (38.3%) is far below the offline routing ablation (93.3%). Trend questions scored 5%; tool selection was 60%. The agent trails the classifier baseline on classification accuracy and trails the RAG baseline on case grounded accuracy despite higher overall score.
