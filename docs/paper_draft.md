...
After cleaning, **240,640** accident records remained (273,614 raw; exclusions documented in `docs/REPRODUCTION.md`). An 80/20 stratified split yielded 192,512 training and 48,128 test records across ten `DEGREE_INJURY_CD` classes (codes 01–10).
...
A naive baseline that always predicts the most common injury degree code (code 03, roughly 36.4% of the cleaned dataset) would score about 36.4% accuracy; the classifier's 57.4% is a sanity check that the model beats that trivial majority-class rule by about 21 percentage points, not a comparison to a competing method.
...
A paired McNemar exact test comparing the agent against each baseline on the same 60 questions found neither difference to be statistically significant. Agent versus classifier baseline: 7 discordant pairs (6 in the agent's favor, 1 in the classifier's), exact p = 0.125. Agent versus RAG baseline: 30 discordant pairs (18 in the agent's favor, 12 in the RAG baseline's), exact p = 0.362. At n = 60, the observed accuracy gaps should be read as suggestive rather than confirmed differences between systems.

All three systems attempt all 60 questions. The live agent exceeds both single-tool baselines on overall accuracy but remains weak on trend questions (5%), where tool outputs require correct numeric parsing from natural language. It **underperforms the classifier baseline on classification questions** (85.0% vs 90.0%) and **underperforms the RAG baseline on case grounded questions** (25.0% vs 85.0%); the overall 38.3% figure reflects breadth across categories, not dominance on any one type (see significance testing above).
...
This paper makes three contributions, now supported by implemented code and measured results. First, it introduces a tool augmented LLM agent architecture for reasoning over the MSHA accident and injury dataset, combining a structured classifier, a statistical trend tool, and a retrieval tool over incident narratives, following the transparent, framework free orchestration design demonstrated by Lu (2026) in a different industrial domain. Second, on a fixed 60 question benchmark where every system attempts every question, the live Groq agent achieves 38.3% overall accuracy versus 30.0% for the classifier baseline and 28.3% for the RAG baseline, with tool selection correctness at 60%; a McNemar exact test found neither paired comparison statistically significant at n = 60 (Section 6.2). The agent underperforms the classifier on classification questions (85.0% vs 90.0%) and underperforms the RAG baseline on case grounded questions (25.0% vs 85.0%); its overall lead comes from attempting all three categories rather than excelling on any single one. Third, it prepares the first human evaluation protocol for explanation quality in mine safety AI using the Explanation Satisfaction Scale of Hoffman et al. (2023); participant data collection has not started pending UMaT ethics confirmation.
...
- **Small sample and single Groq run:** The primary Groq benchmark reflects one completed run. LLM outputs are not fully deterministic; repeated runs could shift the reported numbers, particularly with only 60 questions.
- **Statistical significance:** A McNemar exact test on the primary benchmark found neither the agent versus classifier comparison (p = 0.125, 7 discordant pairs) nor the agent versus RAG comparison (p = 0.362, 30 discordant pairs) to be statistically significant at n = 60. The accuracy gaps reported in Section 6.2 are directional, not confirmed. A larger benchmark, or repeated runs, would be needed to establish significance, and this is listed as the most direct next step for follow-up work.
...
- **Human evaluation:** Protocol and draft consent prepared; UMaT ethics requirements not yet confirmed; no participant data collected.
- **Live LLM weakness:** Primary Groq agent accuracy (38.3%; significance testing in Section 6.2) is far below the offline routing ablation (93.3%). Trend questions scored 5%; tool selection was 60%. The agent trails the classifier baseline on classification accuracy and trails the RAG baseline on case grounded accuracy despite higher overall score.
